...
#                            print('self.unknown_bc = ' + str(self.unknown_bc))
    ...
    def saveToCSV(self, time=10, path = "eegToCSV.csv",mode = 'a', delay = 0.1):
        head = []
        if not os.path.isfile(path):
            head = ['poorSignal', 'rawValue', 'highAlpha','lowAlpha','highBeta','lowBeta',
                        'midGamma','lowGamma','delta','theta','attention','meditation']
        end_time = datetime.now() + timedelta(seconds=time)
        with open(path, mode=mode) as brain_wave:
            writeInCSV = csv.writer(brain_wave,delimiter=',')
            writeInCSV.writerow(head)
            print('Reading will start at - ')
            for i in range(3):
                sleep(1)
                print((3-i),'sec')
            while datetime.now() < end_time:
                writeInCSV.writerow([self.poorSignal,
                                    self.rawValue,
                                    self.highAlpha,
                                    self.lowAlpha,
                                    self.highBeta,
